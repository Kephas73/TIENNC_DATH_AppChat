package com.example.kephas73.meera;

public class Database {
    // Bảng User
    public static final String TABLE_USER = "User";
    public static final String TABLE_USER_ID = "id";
    public static final String TABLE_USER_NAME = "userName";
    public static final String TABLE_USER_IMAGE = "imageURL";
    public static final String TABLE_USER_CITY  = "city";
    public static final String TABLE_USER_GENDER  = "gender";
    public static final String TABLE_USER_HOBBY = "hobby";



    // Bảng chat
    public static final String TABLE_CHATS = "Chats";
    public static final String TABLE_CHATS_RECIVER = "reciver";
    public static final String TABLE_CHATS_SENDER = "sender";
    public static final String TABLE_CHATS_MSSG = "mssg";
    public static final String TABLE_CHATS_TIME = "time";


    // Bảng hình
    public static final String FOLDER_IMAGE = "Uploads";


}
